...
class SelectionBox {
    ...
    searchChildInFrustumOnTouch( frustum, object, selectNodes, selectLines ) {
        if ( object.isMesh || object.isLine || object.isPoints ) {
            ...
            if (selectNodes && object.name === 'nodeBigMount') {
    ...
    selectOnCapture(){
        ...
        this.updateFrustum( this.startPoint, this.endPoint );
        this.searchChildInFrustumOnCapture( _frustum, this.scene );
    ...
    searchChildInFrustumOnCapture( frustum, object ) {
        ...
        if ( object.isMesh || object.isLine || object.isPoints ) {
            ...
            if(object.name === 'nodeBigMount'){
                boundingBox = new Box3().setFromObject(object.parent);

                lt.x = boundingBox.min.x; lt.y = boundingBox.max.y;
                rt.x = boundingBox.max.x; rt.y = boundingBox.max.y;
                lb.x = boundingBox.min.x; lb.y = boundingBox.min.y;
                rb.x = boundingBox.max.x; rb.y = boundingBox.min.y;

                if (frustum.containsPoint(lt) && frustum.containsPoint(rt) && frustum.containsPoint(lb) && frustum.containsPoint(rb)) {
        ...
        if ( object.children.length > 0 ) {
            ...
            for ( let x = 0; x < object.children.length; x ++ ) {
                ...
                this.searchChildInFrustumOnCapture( frustum, object.children[ x ] );
